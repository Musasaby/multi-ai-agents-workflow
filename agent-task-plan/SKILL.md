...
- タスク数が多い場合は、意味的なまとまり(機能グループ)ごとにタスクを並べ、
  `tasks.md` 上でグループ境界が分かるようにする(見出しコメントや小節見出しでよい。
  依存関係の記述だけで十分に整理できている場合は無理にグループ化しなくてよい)。
  このグループ境界は `agent-workflow` skill の「ブランチ分割の推奨」でPR作成・ブランチ
  切替の単位として使われる
